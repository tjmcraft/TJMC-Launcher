		...
		<div className="status-container">
			{isProcessing ?
				(progress > 0 ?
					...
					status == 2 ? <span>R</span> : (
						status == 3 ? <span>F</span> : <span>W</span>
					)
				)
			}
...
const CubeSidebarItem: FC<{
	...
	onDrop = undefined,
		...
		const { name, isProcessing, status } = useGlobal(global => {
			const version = selectInstallation(global, hash);
			return {
				name: version.name,
				type: version.type,
				isProcessing: version.isProcessing,
				status: parseInt(version.status),
			};
		}, [hash]);
		...
		return hash && (
			...
			>
				<span>{name || hash}</span>
				<StatusContainer isProcessing={isProcessing} hash={hash} status={status} />
				<ContextMenu
